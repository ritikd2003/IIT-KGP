"""
this is a Dedalus script for simulating a 2D Vorticity equation with a periodic forcing.
This script demonstrates solving a 2D periodic initial value problem. 
...
problem.add_equation("dt(w)- nu*lap(w) = - skew(grad(psi))@grad(w) +F")
...
# Initial conditions
# Initial conditions
w['g'] = np.random.uniform(-1, 1, w['g'].shape) * 10  # Uniform random initial condition for w
s['g'] = np.random.uniform(-1, 1, s['g'].shape) * 10    # Uniform random initial condition for s
